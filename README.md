...
```
┌──────────────────────────────────────────────────────────────┐
│  IP-камера  ──RTSP──▶  запис / знімки / детектор руху          │
│             ──ISAPI─▶  (Hikvision: апаратні події, зум/фокус)  │
└───────────────┬──────────────────────────────────────────────┘
                │
┌───────────────▼─────────────────  Go-демон  ──────────────────┐
│  camera/   інтерфейс Camera + драйвери (hikvision · generic)   │
│  capture/  планувальник знімків        recorder/  ffmpeg-кліпи │
│  store/    SQLite (метадані)           cleanup/   ретенція     │
│  bot/      Telegram (+ inline-кнопки)  llm/       Azure OpenAI │
│  api/      REST + статика React        config/    YAML + .env  │
└───────────────┬───────────────────────────────┬──────────────┘
                │ REST / медіа                   │ Bot API
┌───────────────▼─────────────┐      ┌───────────▼──────────────┐
│  React + Vite (web/)         │      │  Telegram-бот            │
│  live · галерея · події ·    │      │  команди · пошук · пуш   │
│  налаштування               │      └──────────────────────────┘
└─────────────────────────────┘
```
